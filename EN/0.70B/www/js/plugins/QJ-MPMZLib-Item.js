...
QJ.MPMZ.tl.ex_summonPointDetection = function (result,type) {
    ...
    if (result && result == "defeated") {
           ...
           switch(lang) {
              case 0:
              BulletText = "\\c[2]\\dDCOG[11:2:2:2]\\fs[14]这里无法进行召唤！!";
           break;
           case 1:
              BulletText = "\\c[2]\\dDCOG[11:2:2:2]\\fs[14]ここでは召喚できない！！"
              break;
           case 2:
              BulletText = "\\c[2]\\dDCOG[11:2:2:2]\\fs[14]Summoning is not possible here!!"
              break;
           default:
              BulletText = "\\c[2]\\dDCOG[11:2:2:2]\\fs[14]这里无法进行召唤！!";
           break;
           ...
           var posY = this.inheritY() - 36;	
           $gameTemp.drill_GFTT_createSimple( [posX,posY], BulletText, 2, 9, 90 );
...
//塔罗牌激活演出
QJ.MPMZ.tl.ex_chantingTarotCard = function(imgIndex,itemId) {
...
QJ.MPMZ.tl.ex_fireBirdBlessingEffect = function() {
    ...
    if ($gameSystem.hasGameTimeEvent("state105")) {
        $gameParty.leader().addState(105);
        $gameSystem.adjustGameTimeEventDelay('state105', 30, true);
    } else {
  ...
  FacePunchGloves: function() {
    ...
    return {
      ...
      existData: [
        ...
        {
          t:['G',['"enemy"']],
          a:['F',QJ.MPMZ.tl.ex_toEnemyPunchAttack,[ bulletDamage, {orbitingDamage:true, noHitEffect:true, noDurLoss:true, specifyAddedStatus:true} ]],
          p:[-1,false,true]
        },
      ],
      collisionBox: ['C', 16]
...
QJ.MPMZ.tl.ex_piggyBankLosesMoney = function(target) {
    ...
    QJ.MPMZ.tl.ex_unequipPlayerSpecifiedEquipment(armor,true);
