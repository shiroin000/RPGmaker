...
QJ.MPMZ.tl.ex_playerAttackModeDetection = function() { 
		...
		if ($gamePlayer.isStealthMode()) return;
		//$gameParty.leader().equips()[0].durability -= 180;
...
QJ.MPMZ.tl.ex_playerWeaponDurabilityText = function() {
			 ...
			 let BulletText = Math.floor(durRate) + "%";
			 //let posX = 1900 / $gameScreen.zoomScale();
			 //let posY = 225 / $gameScreen.zoomScale();
    ...
    shadowOffsetY:0
            ...
            position: [['S',posX], ['S',posY]],
			groupName: ['weaponDurabilityReminder'],
            initialRotation: ['S', 0],
            imgRotation: ['F'],
...
QJ.MPMZ.tl.ex_playerArmorImage = function(id,fadeOut) {
    ...
    if ($gameParty.leader().equips()[id]) {
		...
		let text = QJ.MPMZ.tl.ex_playerSetItemDescription($dataArmors[index]);
            text = text.split("\n");
        index = $dataArmors[index].iconIndex;
...
QJ.MPMZ.tl.ex_abyssTimeFlow = function() {
	...
	// 法杖吟唱状态
    if ($gameParty.leader().isStateAffected(68)) {
        const leader = $gameParty.leader();
        const equippedWeapon = leader.equips()[0];
        const clearStatesAndEffects = () => {
        ...
        const weaponType = $dataWeapons[equippedWeapon.baseItemId].wtypeId;
        const staffTypes = [5, 6, 7];
...
QJ.MPMZ.tl.ex_playerDamageCheck = function(baseDamage,damageType,effectId,probability,effectValue1,effectValue2) {
  ...
  //伤害演出和实际受伤
   ...
   //装备效果判定
    if (finalDamage > 5) { 
	  QJ.MPMZ.tl.ex_playerHitTriggerEffect();
	}
  //重伤判定
  ...
  if ( effectId <= 0 ) return;
     probability = probability * $gameParty.leader().stateRate(effectId);
	 if (probability < Math.randomInt(101)) return; 
	// 中毒
    if (effectId === 5) { 
...
// 玩家受击触发特效
QJ.MPMZ.tl.ex_playerHitTriggerEffect = function() {
...
QJ.MPMZ.tl.ex_PlayerHitCheck = function() { 
    ...
    //防止玩家没死成
   if ($gameActors.actor(1).isStateAffected(1) && !$gameSystem._ZzyTWFTheWorlding ) {
	   var data = {
        'type': "公共事件",
        'pipeType': "并行",
        'commonEventId': 4,
...
QJ.MPMZ.tl.ex_playerMeleeAttackTrigger = function() {
 ...
 // 手柄检测
 if ( navigator.getGamepads() && navigator.getGamepads()[0] !== null ) {
	 GamepadsAttack = !Input.drill_isPadPressed('LT') && (Input.drill_isPadPressed('右摇杆上') || Input.drill_isPadPressed('右摇杆下') || Input.drill_isPadPressed('右摇杆左') || Input.drill_isPadPressed('右摇杆右'));
...
QJ.MPMZ.tl.ex_senpuuGiriThrow = function(GamepadsAttack) {
    ...
    var senpuuGiriThrow = QJ.MPMZ.Shoot({
        ...
        existData:[
			...
			{t:['G',['"enemy"','"object"']],a:['F',QJ.MPMZ.tl.ex_senpuuGiriHold]},		
...
//旋风斩三段效果
QJ.MPMZ.tl.ex_senpuuGiriHold = function() {
	if(!$gameParty.leader().equips()[0]) {
    ...
    var senpuuGiriHold = QJ.MPMZ.Shoot({
        ...
        existData:[
			...
			//{t:['G',['"enemy"','"object"']],a:['C',155,[weaponDamage,0,0,0]],p:[-1,true,true]},
			{t:['G',['"enemy"','"object"']],a:['F',QJ.MPMZ.tl.ex_toEnemyAttack,[weaponDamage,{}]],p:[-1,false,true],c:['T',0,6,true]},
        ],
		moveType:['S',0],
        z:zz,
		collisionBox:['R',8,64],
        judgeAccuracyRotation:36,//判定精度，防止挥剑速度太快导致无法攻击到敌人
		moveF:[
		],
        trailEffect:[{
            img:['L',0.5,1,0,0.999999999,0.4,0,0,0],
