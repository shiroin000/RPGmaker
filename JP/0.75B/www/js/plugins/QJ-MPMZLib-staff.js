...
QJ.MPMZ.tl.ex_staffAim = function() {
    ...
    QJ.MPMZ.Laser({
	   ...
	   imgPoint:'null1',
	   existData:[
            {t:['S','!TouchInput.drill_isLeftPressed()',true],a:['S','$gamePlayer.drill_ECE_endSustainingFloating();QJ.MPMZ.tl.ex_staffAlwaysVisible()'],p:[-1,false,true],d:[0,20],c:['S','this.time > 30']},   			
       ],	
       moveF: [
...
QJ.MPMZ.tl.ex_stickSpecialAttack = function() {
    ...
    if (skillMatch) {
        skillMatch = skillMatch[1];
        var code = "QJ.MPMZ.tl.ex_" + skillMatch + "Circle.call(this)";
...
QJ.MPMZ.tl.ex_meteorStrikeCircle = function() {
 ...
 if ($gameParty.leader().equips()[0]) {
   ...
   var magicDamage = 2 * Math.floor(baseDamage * magicScale * damageCorrection);	
	$gameParty.leader().equips()[0].durability -= magicDamage;
	// 法杖因耐久度归零破损
...
QJ.MPMZ.tl.ex_fireballImpactBullet = function() {
	...
	// 耐久度结算
 if ($gameParty.leader().equips()[0]) {
   var durabilityDamage = Math.floor(30 + (0.4 * chahuiUtil.getVarianceDamage(2)));
	$gameParty.leader().equips()[0].durability -= durabilityDamage;
	// 法杖因耐久度归零破损
	if ($gameParty.leader().equips()[0].durability <= 0) {
		$gamePlayer.drill_ECE_endSustainingFloating();
		$gameScreen._particle.particleClear('mahoujin_c-P');
...
QJ.MPMZ.tl.ex_blackHoleCircle = function() {   
  if(!this || this._chargeCounter < 60) return;
   ...
   var baseDamage = 12 + (0.5 * chahuiUtil.getVarianceDamage(2));
