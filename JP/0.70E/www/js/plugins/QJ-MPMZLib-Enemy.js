  ...
  if (args && args.target && args.target instanceof Game_Event) {
    ...
    if (weaponDamage > 0) {
	 ...
	 // 伤害输入预处理-敌人标签识别
      ...
      // 若允许潜行加成
      if (allowSneak && $gameParty.leader().hasSkill(11)) {
        if (!args.target._shouldAlert && !Galv.DETECT.los($gamePlayer, args.target)) {
          ...
          let sneakDamageBonus = 1;
          sneakDamageBonus += $gameParty.leader().skillMasteryLevel(11);
          sneakDamage *= sneakDamageBonus;
...
//敌标准投掷武器
QJ.MPMZ.tl.ex_enemy_weaponThrow = function(EID, effect1 = 0, effect2 = 0) {
...
QJ.MPMZ.tl.ex_collisionDarkDevourerBullet = function(target) { 
 if(this.time < 20) return;
    if (target instanceof Game_QJBulletMZ) {
       if (target) {
	     target.setDead({t:['Time',0],d:[1,15,0.1]})			   		 
    }
		 return;
	 }
