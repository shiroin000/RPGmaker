...
//创造谏言符文
QJ.MPMZ.tl.ex_playerCreateAnuszRune = function() {
	
var maxLength = 36; // 最大字符数
var userInput = "";

while (true) {
    userInput = prompt("请输入文字（最多 " + maxLength + " 个字符）:", userInput || "");
    ...
    if (userInput.length > maxLength) {
        alert("输入的文字太长了！请重新输入（最多 " + maxLength + " 个字符）");
    } else {
        break; // 输入符合要求，跳出循环
    }
}
	console.log(userInput.length);	
    var posX = parseFloat($gamePlayer.centerRealX().toFixed(1));
    var posY = parseFloat($gamePlayer.centerRealY().toFixed(1)); 
	QJ.MPMZ.tl.ex_createAnuszRune(posX,posY,userInput,true);
};
...
chahuiUtil.autoUpdate = function(version) {
      ...
      var url = "https://script.google.com/macros/s/AKfycby-MTYAdRZw3ZJX5J3-YQHicQ-vvatFSGahl0ZyBW6pN97BVwrN9EsryYgiop5vXsO1/exec"
