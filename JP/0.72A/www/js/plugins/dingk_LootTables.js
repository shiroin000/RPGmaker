...
dingk.Loot.specifiedLocationGenerateDrops = function (enemyId, XX, YY) {
    ...
    drops.forEach(function (drop) {
        if ($gameParty.leader().hasSkill(40)) { // 招财猫自动收集
            let itemType;
            if (DataManager.isItem(drop)) {
                itemType = 0; // 物品
            } else if (DataManager.isWeapon(drop)) {
            ...
            img:['I',drop.iconIndex],
            initialRotation:0,
            imgRotation:['S',0],
            moveType:['TP',6,10,15],
            existData:[{t:['P'],c:['T',30,0,true],a:['F',QJ.gainItemEx,[itemType,drop.id,1]],d:[1,15,1.1]},
...
dingk.Loot.generateEnemyDrops = function ( target ) {
    ...
    if (DataManager.isItem(drop)) {
		...
		QJ.MPMZ.itemGiverCharacter(itemType,drop.id,target._eventId,1);
		 } else {
