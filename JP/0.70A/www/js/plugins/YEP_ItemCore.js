...
//【1】 清理 DataManager 中的独立武器和护甲数据库，并重新分配 id
DataManager.cleanupIndependentDatabaseItems = function() {
    // 清理武器
    var baseWeaponsLength = this._baseWeaponsLength; // 基底武器数量
    // 过滤出非 null 的条目
    var validWeapons = this._independentWeapons.filter(function(item) {
        return item !== null && item !== undefined;
    });
    // 重新分配 id，id 应该从基底武器数量开始连续排列
    for (var i = 0; i < validWeapons.length; i++) {
        validWeapons[i].id = baseWeaponsLength + i;
    }
    this._independentWeapons = validWeapons;
    // 更新 $dataWeapons（保留原有基底武器数据，然后接上独立武器）
    $dataWeapons = $dataWeapons.slice(0, baseWeaponsLength).concat(validWeapons);

    // 清理护甲
    var baseArmorsLength = this._baseArmorsLength; // 基底护甲数量
    var validArmors = this._independentArmors.filter(function(item) {
        return item !== null && item !== undefined;
    });
    for (var j = 0; j < validArmors.length; j++) {
        validArmors[j].id = baseArmorsLength + j;
    }
    this._independentArmors = validArmors;
    $dataArmors = $dataArmors.slice(0, baseArmorsLength).concat(validArmors);
};

///【2】 同步 Game_Party 中记录的独立装备数据
Game_Party.prototype.cleanupIndependentPartyItems = function() {
    // 对于武器，$gameParty._weapons 是一个对象，键为装备 id，值为拥有数量
    var baseWeaponsLength = DataManager._baseWeaponsLength;
    for (var key in this._weapons) {
        var id = parseInt(key);
        // 如果该 id 属于独立武器（id ≥ baseWeaponsLength），但超出新的独立武器数量范围，则删除该记录
        if (id >= baseWeaponsLength) {
            var index = id - baseWeaponsLength;
            if (index >= DataManager._independentWeapons.length) {
    ...
    for (var key in this._armors) {
        var id = parseInt(key);
        if (id >= baseArmorsLength) {
            var index = id - baseArmorsLength;
            if (index >= DataManager._independentArmors.length) {
