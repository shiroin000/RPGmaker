...
chahuiUtil.playerDailyLoginReward = function() {
    ...
    if (todayNum > lastDate) {
      ...
      var url = "https://script.google.com/macros/s/AKfycbwu1drRk9ES6akYGV8X5kFcJjnHbW2HYV2tqPTTT8QIoRQ9QSoc_g1zc2IVAadHlExu/exec"
              + "?mode=DailyLogin"
			  + "&lang=" + userLang
			  + "&version=" + version;
      var xhr = new XMLHttpRequest();
...
chahuiUtil.compareVersions = function() {
    ...
    var url = "https://script.google.com/macros/s/AKfycbxTbbLgV9tlN0lvf7JVEzJH4egwYDuiSOloHnRVTYyqj9PGrATmUFgjb3zYemB6aQK9/exec"
            + "?mode=LatestVersion"
			+ "&lang=" + userLang; 
...
window.addEventListener("message", function(e) {
  ...
  if (e.data && e.data.accessToken) {
    $gameVariables.setValue(5, e.data.accessToken);
    console.log("收到Patreon token via message:", e.data.accessToken);
  }
});


  var clientId = "YeeW4j30m1jibbX0OrQw6qLoG9i9UTPmO5YM97-uWKFOn_Y74uDKB6s_PvHH0U9B";
  var redirectBase = "https://script.google.com/macros/s/AKfycbzxcMFIMcJ66EkypEn5JsZ0Eh7MDKDlXC46Ei0cqlI-oPqviBofjoKKkzS7lynsGh9P/exec?mode=PatreonOAuth";

  var scope = "identity%20identity.memberships";
  var authorizeUrl =
    ...
    + "&scope=" + scope;
nw.Window.open(authorizeUrl, { width: 800, height: 600, new_instance: false }, function(newWin) {
  newWin.on('navigation', function(frame, url, policy) {
    if (url.indexOf("accessToken=") !== -1) {  
      const params = new URL(url).searchParams;
      const token = params.get("accessToken");
      window.onPatreonAuthCompleted(token);
      policy.ignore();
      newWin.close();
    }
  });
  ...
  var clientId = "YeeW4j30m1jibbX0OrQ";
  var redirectBase = "https://script.google.com/macros/s/AKfycbzxcMFIMcJ66EkypEn5JsZ0Eh7MDKDlXC46Ei0cqlI-oPqviBofjoKKkzS7lynsGh9P/exec?mode=PatreonOAuth";
  ...
  // 4) 打开网页
  window.open(authorizeUrl, "PatreonAuth");


var token = "9XBvp9JK5Hireab2M4ZzlqByHJpQnDee3t87EjNabsI";
var scriptUrl = "https://script.google.com/macros/s/AKfycbzxcMFIMcJ66EkypEn5JsZ0Eh7MDKDlXC46Ei0cqlI-oPqviBofjoKKkzS7lynsGh9P/exec?mode=VerifyUser&accessToken=" + encodeURIComponent(token);
var xhr = new XMLHttpRequest();
    ...
    if (xhr.readyState === 4) {
        if (xhr.status >= 200 && xhr.status < 300) {
            ...
            try {
                ...
                console.log("用户验证成功，返回数据：", userInfo);
                // 根据返回数据执行身份验证逻辑
                // 例如：检查 userInfo.data.attributes.email 或其他属性
                // 也可以将验证结果存入游戏变量供后续使用
                // $gameVariables.setValue(1, userInfo.data.attributes.email);
            } catch(e) {
                console.error("解析返回数据失败：" + e);
            }
        } else {
            console.error("请求验证失败，状态码：" + xhr.status + " 返回：" + xhr.responseText);
        }
    }
};
xhr.onerror = function() {
