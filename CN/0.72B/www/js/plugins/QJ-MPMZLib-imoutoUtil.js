...
QJ.MPMZ.tl._imoutoUtilStateText = {
        ...
        33: { // 乘凉状态
            ...
            description: {
                ...
                2: "\\fs[14]✦\\fi\\c[110]She’s leisurely licking an ice pop,\nbut she seems too unguarded…"
            }
        },
        28: { // 玩游戏状态
            setValue: {
        ...
        32: { // 肚子饿状态
            ...
            description: {
                ...
                2: [
                    "\\fs[14]✦\\fi\\c[110]Sister is quietly waiting for dinner",
                    "\\fs[14]✦\\fi\\c[110]Sister seems a bit worried \nabout the dinner taste…",
                    "\\fs[14]✦\\fi\\c[110]Sister appears like she wants to help"
                ]
            }
        }
};
...
QJ.MPMZ.tl._imoutoUtilCheckInitialization = function() {
    ...
    var imoutoUtil = QJ.MPMZ.Shoot({
		...
		moveF:[
		  [60,180,QJ.MPMZ.tl._imoutoUtilkokanBarFades],  //监听体力HUD隐藏
		  [30,20,QJ.MPMZ.tl._imoutoUtilOniiChansHpBarFades],   //监听体力HUD隐藏
		  [60,600,QJ.MPMZ.tl._imoutoUtilautoUpdataCheck]  // 自动更新检测
		],
    });	
...
QJ.MPMZ.tl._imoutoUtilautoUpdataCheck = function() {
	...
	if ($gameStrings.value(1).trim() !== "") return;
...
QJ.MPMZ.tl._imoutoUtilCallSisterOver = function() {
	...
	let forbid1 = $gameMessage.isBusy() || $gameMap.isEventRunning() || $gameSwitches.value(14) || $gameScreen.isPointerInnerPicture(81);
...
QJ.MPMZ.tl._imoutoUtilSleepEventListener = function() {
	...
	console.log((currentMinutes >= finalTargetMinutes) || blueKonpeito);
...
QJ.MPMZ.tl._imoutoUtilImoutoOutfitReset = function() {
	...
	$gameActors.actor(2).changeEquipById(2, newPanties);
	}	
	// 复原睡衣和短裤
...
QJ.MPMZ.tl._imoutoUtilImoutoDrinksColaFizzingUp = function() {
    ...
    // 可乐喷射动画
	if (this._frames == 13) {
		this._loopType2 = this._loopType2 || 8;
		if (this._loopType2 > 1) {
			this._loopType2 -= 1;
			this._frames = 10;
			this._coolDown -= 2;
	...
	if ( this._frames == 15 ) {
		this._frames = 0;
		this._coolDown += 120;	
        this._loopType1	= undefined;
        this._loopType2	= undefined;
		this._fizzingUp	= undefined;
		this._switchFrames = undefined;
    ...
    var Skinship = QJ.MPMZ.Shoot({
        ...
        existData: [
            { t: ['SW', 46, false] },
            {
                ...
                a: ["S", "$gameMap.event(4).steupCEQJ(1)"]
            }
        ],
        moveF: [
