...
const LANG_CODE = ["CN", "JP", "EN", "RU", "FR", "VN", "ES"];
...
// --- 根据 ConfigManager.language 加载对应语言的数据文件 ---
  ...
  DataManager.loadMapData = function(mapId) {
    ...
    if (mapId > 0) {
	  ...
	  const mapIdArray = [3,6,7,9,18,19,20,24,25];
				...
				*/
				if (ConfigManager.language === 3) {
				    ...
				    let text =  ["La version française est encore en cours de traduction et de remplissage du texte. ",
						...
						text = text.join('\n');
                    alert(text);				
				} else if (ConfigManager.language === 5) {
				    let text =  ["Bản dịch tiếng Việt của trò chơi hiện vẫn đang trong quá trình dịch và bổ sung.",
						...
						text = text.join('\n');
                    alert(text);				
				} else if (ConfigManager.language === 6) {
				    let text =  ["La versión en español sigue en traducción y adición de textos.",
                                 "Por ello, algunos textos pueden faltar o producir errores en el juego.",
                                 "¡Gracias por su paciencia hasta las próximas actualizaciones!"];
						text = text.join('\n');
                    alert(text);				
				}
...
Game_Interpreter.prototype.showMapEventDialogue = function(idx, subIdx) {
        ...
        if ( textArray.length === 0 ) {
         ...
         textArray = ["\\c[10]Text missing — unable to display!",
		              "This may be caused by an incomplete translation.",
					  "Please try switching to another language! "
					  ];
        }		
...
chahuiUtil.multilingualCompatibleDisplayText = function(textArray, isBlackBackground) {
  ...
  chunks.forEach(function(lines, pageIndex) {
	...
	let text = lines.join('\n');
    //if (Utils.isOptionValid("test")) DrillUp.g_DNB_nameBox_suffix = `\\fn[UoqMunThenKhung]\\fb\\dDCOG[8:6:0:0]${name}`;
	//text = cn2tw(text);
...
DataManager.reloadLanguage = async function(needSave=false) {
  ...
  const nav = navigator.language.toLowerCase();
  // 自动繁化
  if (nav.includes('tw') && curLang == 0)  needTW = true;
...
chahuiUtil.loadMapEventDialogue = function (Specified, mapId) {

  let needTW = false;
  const nav = navigator.language.toLowerCase();
  if (nav.includes('tw'))  needTW = true;
  
  if (!Specified) Specified = ConfigManager.language;
  
  let mapIdRaw = $gameMap.mapId();                 // 4
  if (mapId) mapIdRaw = mapId;
...
(function () {
  ...
  // 同步转换函数（UMD full.min.js 返回的就是函数）
  window.cn2tw = window.cn2tw || (window.OpenCC ? OpenCC.Converter({ from:'cn', to:'tw' }) : (s=>s));

  // 初始化：兼容返回 Promise 的构建
  TZ.initOpenCC = function() {
    if (!window.OpenCC) return;
    try {
      t2s = OpenCC.Converter({ from:'tw', to:'cn' });
      s2t = OpenCC.Converter({ from:'cn', to:'tw' });
      if (typeof t2s !== 'function') t2s.then(fn => t2s = fn);
      if (typeof s2t !== 'function') s2t.then(fn => s2t = fn);
    } catch(e) {}
  };
  TZ.initOpenCC();

  // —— 原有：检测是否包含繁体 —— //
  TZ.hasTraditional = function (str) {
    if (!str) return false;
  ...
  TZ.cn2twJson = function(obj, opts) {
    const conv = (typeof s2t === 'function') ? s2t : (typeof window.cn2tw === 'function' ? window.cn2tw : (x=>x));
    opts = opts || {};
