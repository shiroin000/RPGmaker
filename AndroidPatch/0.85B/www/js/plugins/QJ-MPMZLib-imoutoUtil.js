...
QJ.MPMZ.tl._imoutoUtilGenerateOptionText = function (idx, specified) {
  const eid   = String(specified ?? this?._eventId ?? "");
	...
	if ($gameSystem.hour() > 5 && $gameSystem.hour() < 17) {
     ...
     switch (weather) {
       ...
       break;	   
		...
		// 夜晚场景
		 randomBgmArray = ["できたてブレッツェルはいかが？","Cafe-et-Croissant","ほんのりいい感じなピアノ","ふんわりシフォンとカプチーノ","Strahlburg", 
...
QJ.MPMZ.tl._imoutoUtilDisplayStatusHud = function () {
        ...
        */
		...
		if (isSleep)  pid = 3;
		if (isBath)   pid = 8;
		if ($gameMap.mapId() == 4 && isBath)  pid = 4;
        if (isBrush)  pid = 9;				
		if (isHot)    pid = 6;	
        ...
        if (picture._opacity > 200 && !picture._drill_MPFP_bean) {
...
QJ.MPMZ.tl._imoutoUtilCheckInitialization = function(forbid) {
    ...
    var imoutoUtil = QJ.MPMZ.Shoot({
		...
		moveF:[
		  ...
		  [60,3600,chahuiUtil.autoUpdataCheck],  // 自动更新检测
...
QJ.MPMZ.tl._imoutoUtilOniiChansHpBarFades = function() {
	...
	// Alt键快进监听
	if ( $gameTemp.drill_OKe_isSpeedGearPressed() ) {
		if (!this._speedGear) {
...
QJ.MPMZ.tl._imoutoUtilSleepEventListener = function() {
    ...
    // 洗澡立绘不触发（需要考虑亲密接触的情况）
    if ($gameActors.actor(2).isStateAffected(25) && !$gameSwitches.value(46)) return;
    ...
    if ( currentMinutes >= finalTargetMinutes ) {
      ...
      if ($gameActors.actor(2).isStateAffected(29) || $gameSelfSwitches.value([$gameMap.mapId(), 14, 'A'])) {
		  if ($gameSwitches.value(46)) {
            QJ.MPMZ.deleteProjectile('skinshipListeners', { a: ['S', `$gameStrings.setValue(36, '');
...
QJ.MPMZ.tl._imoutoUtilSceneNameDisplay = function(text) {
	...
	let scaleY = 0.5 / $gameScreen.zoomScale();
	    posX = 1930 / $gameScreen.zoomScale();;
        posY = 152 / $gameScreen.zoomScale();;
        QJ.MPMZ.Shoot({
            img:"line",
            position: [['S',posX], ['S',posY]],
            initialRotation: ['S', 0],
