...
QJ.MPMZ.tl.ex_enemyFallCheck = function (region, etraData = {}, args) {
	  ...
	  if (args && args.target && args.target instanceof Game_Event) {
		  ...
		  let XX = Math.round(args.target.centerRealX());
		  let YY = Math.round(args.target.centerRealY());
		  
		 if ( $gameMap.regionId( XX, YY ) === region ) {
			 let eid = args.target._eventId;
			 $gameMap.steupCEQJ(133,eid);
		 }
...
QJ.MPMZ.tl.ex_enemyStuckCollisionDamage = function(EID) {
	...
	$gameMap.event(EID).requestAnimation(141);
	let enemyHP = $gameSelfVariables.value([$gameMap.mapId(), EID, 'HP']);
	let realDamage = 100;
	$gameSelfVariables.setValue([$gameMap.mapId(), EID, 'HP'], enemyHP - realDamage);
    SimpleMapDamageQJ.put(2,EID,realDamage,0,-72);
...
QJ.MPMZ.tl.ex_enemyBleeding = function (damage, time) {
    ...
    const removeTagCode = "if($gameMap.event(" + target + ")){$gameMap.event(" + target + ").drill_COET_removeTag('出血标记')}";
    const deadCode      = "!$gameMap.event(" + target + ")";
    const posX          = "if($gameMap.event(" + target + ")){$gameMap.event(" + target + ").screenBoxXShowQJ();}else{$gameMap.displayX()}";
    const posY          = "if($gameMap.event(" + target + ")){$gameMap.event(" + target + ").screenBoxYShowQJ();}else{$gameMap.displayX()}";
    ...
    QJ.MPMZ.Shoot({
        ...
        existData: [
            ...
            { t: ["G", ['"enemy"', '"object"']],
              a: ["F", QJ.MPMZ.tl.ex_enemyBleedingEffects, [damage, target]],
              p: [-1, false, true], c: ["T", 0, 60, true], d: [0, 30] },
        ],
    });
...
QJ.MPMZ.tl.ex_enemyBleedingEffects = function(damage,eventId) {
		 ...
		 $gameMap.event(eventId).requestAnimation(155);	
...
if (!damage) {
 var damage = chahuiUtil.checkWeaponQuality("毒伤");
}
if (!time) {
var time = 480 + Math.randomInt(240);
}
	let stateName = 'enemyPoison' + this._eventId;
   if($gameMap.getGroupBulletListQJ(stateName).length > 0) {
       let BID = $gameMap.getGroupBulletListQJ(stateName)[0];
	   let bullet = $gameMap._mapBulletsQJ[BID];
	      if(!bullet) return;
	       bullet._extraDamage = bullet._extraDamage || 0;
		   bullet._extraDamage += Math.max(1,Math.floor(damage / 4));	 
    ...
    let deadCode = '!$gameMap.event('+ this._eventId + ')';
        let posX = "if($gameMap.event("+this._eventId+")){$gameMap.event("+this._eventId+").screenBoxXShowQJ();}else{$gameMap.displayX()}";
        let posY = "if($gameMap.event("+this._eventId+")){$gameMap.event("+this._eventId+").screenBoxYShowQJ();}else{$gameMap.displayX()}";	
    var xxx = QJ.MPMZ.Shoot({
		 ...
		 existData:[
            {t:['S',deadCode,true],d:[0,30]},		 
			{t:['Time',time],d:[0,30]},
			{t:['G',['"enemy"']],a:['F',QJ.MPMZ.tl.ex_enemyPoisonEffect,[damage]],p:[-1,false,true],c:['T',0,60,true],d:[0,30]},
			],
		 moveF:[
		      [60,60,QJ.MPMZ.tl.ex_enemyPoisonEffect,[damage,this._eventId]]
		    ],
});	
   }

QJ.MPMZ.tl.ex_effectFonts("zhongdu",this._eventId);
       ...
       if($gameMap.getGroupBulletListQJ(stateName).length > 0) {
        ...
        let posY = "if($gameMap.event("+this._eventId+")){$gameMap.event("+this._eventId+").screenBoxYShowQJ();}else{$gameMap.displayX()}";		 
		 ...
		 this._IsDisabledCounter += time;
          ...
          let length = Math.sqrt(this.width() ** 2 + this.height() ** 2);
		     /*
             if (this._boxBodyQJ.type === 1) {
            length = this._boxBodyQJ.w;
              } else {
...
QJ.MPMZ.tl.ex_enemyBurn = function(damage, time, args) {
    ...
    var stateName = 'enemyBurn' + target;

    let posX = `if($gameMap.event(${target})){
                    $gameMap.event(${target}).screenBoxXShowQJ();
                } else {
                    $gameMap.displayX();
                }`;

    let posY = `if($gameMap.event(${target})){
                    $gameMap.event(${target}).screenBoxYShowQJ();
                } else {
                    $gameMap.displayX();
                }`;
...
QJ.MPMZ.tl.ex_enemy_AsabaSpinAttack = function() {
    ...
    QJ.MPMZ.Shoot({
        ...
        existData:[
            {t:['Time',240]},			
			{t:['P'],a:['F',QJ.MPMZ.tl.ex_playerDamageCheck,[8,1,0,0]],c:['T',0,10,true],p:[-1,false,true]},
			{t:['G','"enemy"','"object"'],a:['F',QJ.MPMZ.tl.ex_toEnemyAttack,[8,{immuneItself:eid,orbitingDamage:true,noHitEffect:true,noDurLoss:true}]],c:['T',0,10,true],p:[-1,false,true]},
