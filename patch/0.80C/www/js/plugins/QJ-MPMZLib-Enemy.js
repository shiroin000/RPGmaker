...
QJ.MPMZ.tl.ex_toEnemyAttack = function (weaponDamage, attackData = {}, args) {
  ...
  if (actor.hasSkill(34)) {
    const chance = 5 + 3 * actor.skillMasteryLevel(34);
    if (chance > Math.randomInt(100)) QJ.MPMZ.tl.ex_bombFiendSoul.call(this);
  }
  ...
  // 魔法混合伤害（区分独立攻击不继承法伤）
  if (!attackData.independentAttack && !attackData.magicAttack) {
    const ID = baseId; 
    const inheritedMagic = ($dataWeapons[ID] && $dataWeapons[ID].traits && $dataWeapons[ID].traits[0].dataId === 2);
  ...
  if (enemyHP <= 0) {
    ...
    QJ.MPMZ.tl.ex_enemyDeathEffectResolution.call(this, target);
    return;
  }
...
QJ.MPMZ.tl.BossSpecialNameDisplay = function () {
	...
	var posX = 960 / $gameScreen.zoomScale();
    var posY = 160 / $gameScreen.zoomScale();
	var Scale = 1 / $gameScreen.zoomScale();
    let BulletText = "The Berry Devourer";
	...
	posX = 960 / $gameScreen.zoomScale();
    posY = 70 / $gameScreen.zoomScale();
    BulletText = "★Dwelling in Adventurer's Path★";
