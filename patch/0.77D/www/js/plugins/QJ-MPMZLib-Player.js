...
QJ.MPMZ.tl.ex_playerSetItemDescription = function(item) {
    ...
    if (!description || !description[String(itemId)]) {
		...
		let quantityText = window.systemFeatureText["quantityHeld"][String(ConfigManager.language)].join(); 
  ...
  if (durRate < 0.05) {
	...
	// 武器低耐久度，即将损坏	
	  if (!this._brokenWarning) {
		...
		let imgName = 'durabilityLow' + $gameVariables.value(1);
        QJ.MPMZ.Shoot({
...
QJ.MPMZ.tl.meleeAttack = function() {
	...
	var weaponTrail = "weapon/weaponTrail" + $gameParty.leader().equips()[0].baseItemId;
...
QJ.MPMZ.tl.ex_senpuuGiri = function(GamepadsAttack,Tsubame = false) {
    ...
    // 香蕉大剑特效
    if (!Tsubame && $gameParty.leader().equips()[0].baseItemId === 60) {
