...
QJ.MPMZ.tl._USAUSAUSAUSA = function() {
	...
	let audio = $gameTemp.drill_VI_getAudio( "voice", seName );
	if( audio == null )  { 
	   	this._coolDown = 60;
	   return; 
	}
    let time = 	Math.floor(audio._totalTime*60);
	time -= Math.floor(time * 0.28);
	this._coolDown = time;
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (item && item.note && item.note.includes("<Ingredients:")) {
            ...
            if (ingMatch) {
				...
				if (preference) {
					if (lowerIng.includes(preference)) {
						multiplier = Math.max(199,count);
					}
				}
...
QJ.MPMZ.tl._imoutoUtilImoutoCookingHerselfPhaseTwo = function () {
	...
	if (this._frames >= 5) {		
		...
		this._upend = true;	
        // 投放食材
        QJ.MPMZ.tl._imoutoUtilImoutoAddIngredient.call(this,850,750);
        QJ.MPMZ.tl._imoutoUtilImoutoAddIngredient.call(this,1100,750);
...
// 妹妹投放食材
QJ.MPMZ.tl._imoutoUtilImoutoAddIngredient = function(tarX, tarY) {

  let itemArray = $gameNumberArray.value(15);
  let itemId = itemArray[Math.floor(Math.random() * itemArray.length)];
...
// 食材留在碗里
QJ.MPMZ.tl._imoutoUtilIngredientStayInBowl = function(icon) {
